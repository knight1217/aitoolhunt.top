#!/usr/bin/env python3
"""生成16篇新博客文章到 blog/ 目录 - v3 修复f-string语法问题"""
...
ARTICLES = [
  ...
  <h2 style="font-size:1.5rem;margin-top:40px;margin-bottom:16px;">✍️ Writing & Communication</h2>
  <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:20px;margin-bottom:16px;">
...
def main():
    ...
    for a in ARTICLES:
        ...
        lines.append('      <span style="display:inline-block;background:' + cat_bg + ';color:var(--text-secondary);font-size:0.75rem;padding:4px 10px;border-radius:20px;margin-bottom:12px;">' + cat + '</span>')
        ...
        lines.append('      <span style="color:var(--text-muted);font-size:0.8rem;">📅 ' + a["date"] + " · " + a["read"] + '</span>')
        lines.append('    </a>')
        print('\n'.join(lines))
        print()
